...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DBA - INDEX MONITOR', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'COLETA DIÁRIA DA UTILIZAÇÃO DE ÍNDICES.', 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'LOAD', 
		...
		@command=N'EXEC SP_DBA_INDEXMONITOR @DATABASES = ''USER_DATABASES'', @EXCEPT = ''SQLDAYES,traces''', 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'SEND REPORT', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'EXEC SP_SEND_INDEXREPORT', 
		@database_name=N'DBA_MONITOR', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
